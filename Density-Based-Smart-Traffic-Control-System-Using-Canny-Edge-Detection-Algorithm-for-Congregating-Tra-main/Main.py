...
def yolo():
    from yolov3 import denserlanefile,stringdenser
    global filename
    filename = denserlanefile
    lane["text"] = stringdenser
    messagebox.showinfo("Vehicle Detection", "Vehicle Detection Completed Successfully")
...
def pixelcount():
    ...
    messagebox.showinfo("Pixel Counts", "Total Refrence White Pixels Count : "+str(refrence_pixels)+"\nTotal Sample White Pixels Count : "+str(sample_pixels))
...
font = ('times', 16, 'bold')
title = Label(main, text='                           Density Based Smart Traffic Control System Using Canny Edge Detection Algorithm and Yolov3',anchor=W, justify=CENTER)
...
font1 = ('times', 14, 'bold')
lane = Button(main, text="Vehicle Detection", command=yolo)
lane.place(x=50,y=100)
lane.config(font=font1)  

lane = Label(main, text="Vehicle Detection Not Done",anchor=W, justify=CENTER)
lane.config(bg='yellow4', fg='white')  
lane.config(font=font1)           
lane.place(x=50,y=150)
